...
**Конфиг через окружение (env) — для сервиса:**
```
STATSFEED_SERVE=:47800          # адрес ("" — выключить сетевой сервер)
STATSFEED_TOKEN=...             # токен метрик /stats (пусто — открыто)
STATSFEED_SECURE=1              # авто-токен метрик
STATSFEED_ACTIONS=1             # включить /action (для сопряжённых часов)
STATSFEED_AUTHORIZED="t1 t2 t3" # МАССИВ токенов часов (allowlist экшенов)
```
Флаги остаются как переопределение env. Экшены: добавь токены часов (с экрана
ДОСТУП или из `/identity.json` на часах) в `STATSFEED_AUTHORIZED` или в файл
`UserConfigDir/statsfeed/authorized_watches`.

**Управление доступом часов в трее (Mac/Win):** меню statsfeed → «Часы (доступ)…»
— список разрешённых и запросов на сопряжение, поле для добавления токена.
CLI: `-pending` / `-list-watches` / `-approve <token>`.

По умолчанию (без флагов/env) сервер **открыт** в LAN — часы находят хост сами,
